...
- P1：curriculum reset 协议扩展设计（注意既有 `load_save` 缺 `decision` 字段的缺陷）。
- P2：用真实轨迹构建 Ironclad 实体词表（`tools/m2_build_vocab.py`），扩展 observation 覆盖 `player_powers`/`draw_pile_count`/`discard_pile_count`/intents，接入 GRU 历史。
- P3：PPO 训练环路 + 楼层进度 reward shaping。
