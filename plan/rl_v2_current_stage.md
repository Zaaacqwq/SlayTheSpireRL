...
## v6 地图全图观测（`omni/rl-mapobs` 分支，基于 `omni/rl-deckobs`，2026-07-13）

**动机**：`map_select` 只暴露紧邻可选节点，"进 act 看全图规划路线（两次商店删牌/多次火堆升级/血差避精英）"在观测层面表达不了，训练量再大也学不出来。**不需要改引擎 fork**：现有 `get_map`（`RunSimulator.GetFullMap`）已返回每节点 col/row/房型/children 边/visited/current 及 boss（含 boss 身份 id）。

**实现（全部 Python 侧）**：

- `EngineClient._with_map`：`map_select` 决策时追加一次 `get_map`（每层至多一次，成本可忽略），`rows/boss/current_coord` 并入 raw state 的 `map` 字段；响应非 `map` 类型 fail-closed。
- `observation._map_node_entities`：全图展平为 `map_node` 实体；从当前位置沿 children 边 BFS 标注 `reachable`/`depth`（act 起点无 current 时以首行为根）；boss 的 `id`/`name` 改存 `boss_id`/`boss_name`，避免遮蔽 `entity_key` 的房型键。
- `entities`：ENTITY_KINDS 追加 `map_node`（deckobs 后 10→11），词表 alias `map_node→choice`（房型共享 embedding）；`ENTITY_NUMERIC_DIM` 13→17（追加 visited/current/reachable/depth，非地图实体恒 0）；`candidate_entity_slots` 的地图候选优先指向 `map_node` 孪生节点（带路径上下文），无图时回退 `choice`。
- `checkpoint`：迁移规则从"扩 embedding 行"推广为"也扩 `numeric.weight` 输入列"（零填充只中和新通道；旧状态输出逐位不变，守护测试证明）；迁移仍重置 Adam。
- `tools/m2_probe_mapobs.py`：4 个方向性探针（低血偏好含火路径 / 高金偏好含商店路径 / 低血回避精英路径 / 满血猎精英路径），同一紧邻选择 × 两张受控地图，`--strip-map` 为真盲参照（状态全同，delta 恒 0）。验收标准同 deckobs：**各探针 delta 方向对齐且跨 checkpoint 稳定**，不是 |delta|>0。
- `tools/p0_baseline_hash.py` 支持 `STS2_CLI_ROOT`（worktree 借主仓构建，同 deckobs 的 m2_train 模式）。

**证据**：RL pytest 67 passed（新增 8 个：可达性/深度 BFS、boss 键遮蔽、act 起点根、候选指向与回退、无图不变、数值维度、迁移扩行扩列、迁移后旧状态输出逐位一致）；真引擎冒烟（Ironclad 真实 act1 图）：66 节点全部进观测、7 种房型 0 UNK 0 告警、max_depth 16、候选指向 `map_node`、boss 身份在场；老 v4 checkpoint（13 维/10 kind）迁移加载探针可跑，`--strip-map` 全部 delta 恒 0.0000；**状态 hash 锚点因 map_select 加字段有意变更**，`p0_baseline_hash.json` 重生成连续两次 md5 一致（c5fe2ff0…）。

**排期**：训练排在 v5 结果之后（v5 从 deckobs worktree 训练中不受影响）；届时从 v5 checkpoint 迁移续训或从头训 `m2_v6_mapobs`，先跑 `m2_probe_mapobs.py --strip-map` 对照与探针基线。

## PC（9800X3D + RTX 5080）迁移步骤

1. clone 仓库 + `git submodule update --init`；确认 submodule 在 pin commit。
2. 按 `external/sts2-cli` 上游 setup 复制/patch `sts2.dll`（Steam 安装目录），.NET 9 build；设 `STS2_GAME_DIR`。
3. `python -m venv rl/.venv && pip install -e "rl[dev,train]"` + CUDA 版 torch。
4. 验证：`pytest rl/tests`、`tools/p0_baseline_hash.py` 后 `git diff`（跨平台 hash 应一致，若不一致必须先查明再训练）。
5. 训练：`tools/m2_train.py --device cuda --workers 12`（checkpoint 可从 Mac 直接带过去续训）。
...
- P4：v4 长跑（act1 → full_a0 晋级门槛 dev 0.30）；到点后按上述命令扩展到 5 初始化 + ablation。
- P5：训练产出后跑正式 1,000 test seeds 验收报告。
