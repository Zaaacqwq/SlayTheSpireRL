...
课程实测（修复前那次运行的有效部分，可作参考）：`normal_combat` 第 39 迭代 dev 0.82 晋级（与 v4 的 0.82 平台吻合），`mixed_combat` 第 49 迭代 dev 0.74 一次过 0.60 门槛，随即进入 `boss_combat`。注意本机 `rl/schema/m2_boss_loadouts.json` 已在仓库中，**boss 桥接阶段是启用的**。

对照基线：v4 act1 dev 最佳 12%、avg_floor ~14。验收探针：里程碑 checkpoint 跑 `tools/m2_probe_deckbuild.py --checkpoints milestones` 与 `--strip-deck` 盲参照，要求各探针 delta 方向对齐且跨 checkpoint 稳定。
