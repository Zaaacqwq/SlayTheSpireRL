...
- **其全局流程（full-run）不可用**：`map/acts.py` 仍是**过时的 STS1 配置**（3 幕，boss = TheLich / TheCollector / AwakenedOne，elite = GremlinNob / BookOfStabbing…），且 `RunManager` 实际就在读它。其战斗/遭遇模块是正确的 STS2，但**幕结构是错的**。→ **只能当战斗后端用，不能当整局后端用。**
...
**恰好互补的一点**：我们当前的墙是 **boss 战（0/31 转化）+ 灾难性遗忘**，这是**战斗问题**，而战斗正是该模拟器内容与机制都通过验证的部分——**我们根本不需要它那条坏掉的整局流程**。
