...
| | v5 | v6 |
|---|---|---|
| boss_combat dev | 0.19（320 迭代平台，**从未过门槛**）| **0.36（10 迭代即过）** |
| act1 boss 转化率 | **0/31 = 0%** | **0% → 38% → 19%（已稳定非零）** |
| act1 dev 胜率 | 最好 0.12（约 200 迭代）| 0.10（才 20 迭代）|
| 累计训练量 | 24,720 局 | **2,904 局** |

Fisher 精确检验 **p ≈ 0.002**。

**那堵「boss 墙」——v4 为它造了桥接阶段、收割 loadout、改引擎支持升级卡；v5 为它烧掉 62% 的训练预算（15,408 局）——根本不存在，是 bug 造成的假象。**
